...
import { STAGES, stageIconPath } from './stages.js?v=3';
...
function renderStageCards() {
  ...
  for (const stage of STAGES) {
    ...
    card.innerHTML = `
      <div style="display:flex; align-items:center; justify-content:center; gap:8px; margin-bottom:12px;"><img src="${stageIconPath(stage.stage)}" alt="" style="width:16px; height:auto; filter:brightness(0) invert(1);" /><span style="font-family:'Playfair Display',serif; text-transform:uppercase; letter-spacing:0.05em; font-size:19px; font-weight:400; color:var(--stage-text);">Etapa ${stage.stageRoman}</span></div>
...
function renderStageDetail() {
  ...
  header.innerHTML = `<img src="${stageIconPath(stage.stage)}" alt="" style="width:18px; height:auto;" /><span>Etapa ${stage.stageRoman}</span>`;
